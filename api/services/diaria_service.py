...
def confirmar_presenca_diarista(diaria_id):
    diaria = listar_diaria_id(diaria_id)
    if diaria.status != 3:
        raise serializers.ValidationError("Só é possível executar essa ação em diárias com status 3")
    data_atual = datetime.datetime.now()
    data_diaria = diaria.data_atendimento.replace(tzinfo=None)
    if data_atual <= data_diaria:
        raise serializers.ValidationError("A data atual não pode ser menor que a data de atendimento")
    diaria.status = 4
    diaria.save()
